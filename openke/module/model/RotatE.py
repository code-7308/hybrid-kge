...
class RotatE(Model):
	...
	def __init__(self, ent_tot, rel_tot, dim = 100, margin = 6.0, epsilon = 2.0, pretrain_model=None):
		...
		if pretrain_model is None:
			...
			ent_weights = torch.FloatTensor(pretrain_model["ent_embeddings.weight"])
			rel_weights = torch.FloatTensor(pretrain_model["rel_embeddings.weight"])
			self.ent_embeddings = nn.Embedding.from_pretrained(ent_weights, freeze=False)
			self.rel_embeddings = nn.Embedding.from_pretrained(rel_weights, freeze=False)
			self.ent_embedding_range = nn.Parameter(
				torch.FloatTensor(pretrain_model["ent_embedding_range"]),
				requires_grad=False
			)
			self.rel_embedding_range = nn.Parameter(
				torch.FloatTensor(pretrain_model["rel_embedding_range"]),
				requires_grad=False
			)
